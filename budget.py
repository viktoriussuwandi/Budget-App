import pandas as pd
import numpy as np
import files, json

class Category:

  def __init__(self, category=None):
    self.ledger        = []
    self.data_saved    = []
    self.category      = category if category is not None else None
    self.balance       = 0
    self.form_ledger = {'amount' : 0, 'description' : ''}
    self.format_saved_transaction = {
      'category'   : self.category,
      'activity'   : '', 'amount' : 0,
      'description': '', 'status' : False, 'balance': 0
    }
    
    fileName           = 'transactions'
    self.file          = files.Files(fileName)

  def __str__(self):
    return f'Printing {self.category}'

  def deposit(self, amount, desc = None):
    form  = self.format_saved_transaction
    form['activity']    =  'deposit'
    form['amount']      =  amount
    form['description'] =  desc if desc is not None else 'deposit'
    form['status']      =  True
    self.balance                += amount
    form['balance']     =  self.balance
    self.save_data(form)
    
    form_ledger = self.form_ledger
    form_ledger['amount']      : form['amount']
    form_ledger['description'] : form['description']
    return form_ledger

  def refresh_data(self):
    self.file.refresh_data()
    reload_data = self.file.data
    if reload_data is not None :
      self.data_saved = self.file.data
      # format_ledger = [ 
      #   { 'amount' : d['amount'], 'description' : d['description'] } 
      #   for d in self.data_saved[self.category]
      # ];
      # self.ledger = format_ledger
      # print(json.dumps(self.ledger, indent = 4))
      print(json.dumps(self.data_saved, indent = 4))
      

  def save_data(self, form) :
    is_saved = self.file.save_data(form)
    if is_saved == True : self.refresh_data()
      # for (key,val) in form.items() : print(f'{key} : {val}')
      
def create_spend_chart(categories):
  print('create spend chart')

  # -----------------------------------------------------------------------------------
  #   def withdraw(self, amount, desc = None):
  #   form = self.format_saved_transaction
  #   form['activity']    = 'withdraw'
  #   form['amount']      = amount
  #   form['description'] = desc if desc is not None else 'withdraw'
  #   form['status']      = True if (self.balance > amount) else False
  #   self.balance                = self.balance if form['status'] == False else (self.balance - amount)
  #   form['balance']     = self.balance
  #   # for (key,val) in form.items() : print(f'{key} : {val}')
  #   # print('\n')
  #   return form['status']

  # def get_balance(self, ):
  #   return self.balance

  # def transfer(self, amount, to):
  #   withdraw_status = self.withdraw(amount, f"Transfer to {to}")
  #   transfer_status = withdraw_status
  #   if withdraw_status == True : 
  #     self.category = to
  #     self.deposit(amount, "Initial Deposit")
  #   return transfer_status

  # def check_funds(self, amount) :
  #   return amount > self.balance
